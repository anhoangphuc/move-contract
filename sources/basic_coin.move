module my_addr::BasicCoins {
    ...
    /// Error codes
    const ERR_BALANCE_NOT_EXIST: u64 = 101;
    const ERR_BALANCE_EXISTS: u64 = 102;
    const EINSUFFICIENT_BALANCE: u64 = 1;
    const EALREADY_HAS_BALANCE: u64 = 2;
    const EEQUAL_ADDR: u64 = 4;

    /// Mint `amount` called val tokens on contracts
    ...
    public fun deposit(acc_addr: address, coins: Coins) acquires Balance {
        assert!(balance_exists(acc_addr), ERR_BALANCE_NOT_EXIST);
    ...
    public fun withdraw(acc: address, value: u64): Coins acquires Balance {
        assert!(balance_exists(acc), ERR_BALANCE_NOT_EXIST);
    ...
    fun test_use_some_coins(acc: signer) acquires Balance {
        ...
        assert!(coins_5.val == 5, EINSUFFICIENT_BALANCE);
